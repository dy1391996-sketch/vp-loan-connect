...
function BasicStep({ form, update, otpStarted, otpVerified, busy, requestOtp }: StepProps & { otpStarted: boolean; otpVerified: boolean; busy: boolean; requestOtp: () => void }) {
  return <div><StepTitle title="Tell us how much you need" description="Start with your loan need and basic contact details. Your free result comes before payment." /><div className="mt-7 grid gap-5 sm:grid-cols-2"><Field label="Full name" required><Input autoComplete="name" value={form.fullName} onChange={(e) => update("fullName", e.target.value)} placeholder="Your full name" /></Field><Field label="WhatsApp mobile number" required hint="10-digit Indian mobile number"><Input className="number-field" inputMode="numeric" autoComplete="tel" maxLength={10} value={form.mobile} disabled={otpVerified} onChange={(e) => update("mobile", e.target.value.replace(/\D/g, "").slice(0, 10))} placeholder="98XXXXXXXX" /></Field><Field label="Email address" required hint="We will send the OTP to this email"><div className="flex gap-2"><Input type="email" autoComplete="email" value={form.email} disabled={otpVerified} onChange={(e) => update("email", e.target.value.trim())} placeholder="name@example.com" />{!otpVerified ? <Button type="button" variant="secondary" className="shrink-0" disabled={busy} onClick={requestOtp}>{busy ? <Loader2 className="animate-spin" size={17} /> : null}{otpStarted ? "Resend OTP" : "Verify email"}</Button> : <span className="grid min-w-12 place-items-center rounded-xl bg-brand-100 text-brand-700"><Check size={19} /></span>}</div>{otpStarted && !otpVerified ? <p className="mt-2 text-xs text-slate-500">Enter the email OTP in the secure MSG91 window to complete verification.</p> : null}</Field><div className="hidden sm:block" /><Field label="State" required><Input autoComplete="address-level1" value={form.state} onChange={(e) => update("state", e.target.value)} placeholder="e.g. Uttar Pradesh" /></Field><Field label="City" required><Input autoComplete="address-level2" value={form.city} onChange={(e) => update("city", e.target.value)} placeholder="e.g. Greater Noida" /></Field><Field label="Residential address" required><Input autoComplete="street-address" value={form.residentialAddress} onChange={(e) => update("residentialAddress", e.target.value)} placeholder="House / building, area and landmark" /></Field><Field label="PIN code" required><Input inputMode="numeric" maxLength={6} value={form.pinCode} onChange={(e) => update("pinCode", e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="201009" /></Field><Field label="How much money do you need?" required hint="Choose a quick amount or enter your own amount from ₹5,000 to ₹5,00,000"><div className="mb-3 grid grid-cols-3 gap-2">{["10000", "25000", "50000", "100000", "200000", "500000"].map((amount) => <button key={amount} type="button" onClick={() => update("loanAmount", amount)} className={cn("rounded-xl border px-3 py-3 text-sm font-extrabold transition", form.loanAmount === amount ? "border-brand-600 bg-brand-100 text-brand-800" : "border-line bg-white text-navy-950 hover:border-brand-500")}>{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR", maximumFractionDigits: 0 }).format(Number(amount))}</button>)}</div><Input className="number-field" type="number" inputMode="numeric" min={5000} max={500000} step={1000} value={form.loanAmount} onChange={(e) => update("loanAmount", e.target.value)} placeholder="Enter another amount" /></Field><Field label="Why do you need the loan?" required><Select value={form.loanPurpose} onChange={(e) => update("loanPurpose", e.target.value)}><option value="">Choose a purpose</option><option value="Medical or emergency expense">Medical or emergency</option><option value="Household bills or family need">Household or family need</option><option value="Education expense">Education</option><option value="Home repair or purchase">Home repair or purchase</option><option value="Travel or wedding">Travel or wedding</option><option value="Business working capital">Business need</option><option value="Other personal need">Other personal need</option></Select></Field><Field label="Which loan direction fits you?" required hint="Choose the closest option—this does not lock your final choice."><Select value={form.loanType} onChange={(e) => update("loanType", e.target.value)}><option value="">Choose one</option><option value="PERSONAL">Personal loan / quick cash</option><option value="BUSINESS">Money for my business</option><option value="GOLD">Loan against gold</option><option value="PROPERTY">Loan against property</option><option value="CREDIT_HEALTH">I need help improving eligibility first</option></Select></Field></div><SafetyNote /></div>;
}
