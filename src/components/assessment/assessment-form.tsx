...
import { useEffect, useMemo, useराज्य } from "react";
...
type Formराज्य = {
  ...
  employmentType: string; monthlyIncomeRange: string; durationMonths: string; salaryBankCredit: boolean | null; itrAvailable: boolean | null; gstAvailable: boolean | null; udyamAvailable: boolean | null; sixMonthBankराज्यment: boolean | null;
  ...
  panAvailable: boolean | null; aadhaarAvailable: boolean | null; addressProofAvailable: boolean | null; incomeProofAvailable: boolean | null; bankराज्यmentAvailable: boolean | null; businessRegistrationAvailable: boolean | null; securedAssetAvailable: boolean | null;
  serviceConsent: boolean; marketingConsent: boolean;
};

const initialराज्य: Formराज्य = {
  fullName: "", mobile: "", email: "", state: "", city: "", loanAmount: "", loanPurpose: "", loanType: "",
  employmentType: "", monthlyIncomeRange: "", durationMonths: "", salaryBankCredit: null, itrAvailable: null, gstAvailable: null, udyamAvailable: null, sixMonthBankराज्यment: null,
  existingEmi: "0", activeLoans: "0", cardOutstanding: "0", currentOverdue: null, settledOrWrittenOff: null, creditRange: "",
  panAvailable: null, aadhaarAvailable: null, addressProofAvailable: null, incomeProofAvailable: null, bankराज्यmentAvailable: null, businessRegistrationAvailable: null, securedAssetAvailable: null,
...
export function AssessmentForm() {
  ...
  const [step, setStep] = useराज्य(1);
  const [form, setForm] = useराज्य<Formराज्य>(initialराज्य);
  const [otpStarted, setOtpStarted] = useराज्य(false);
  const [otpVerified, setOtpVerified] = useराज्य(false);
  const [otpToken, setOtpToken] = useराज्य("");
  const [busy, setBusy] = useराज्य(false);
  const [error, setError] = useराज्य("");
  ...
  function update<K extends keyof Formराज्य>(key: K, value: Formराज्य[K]) {
  ...
  function validateCurrentStep() {
    ...
    if (step === 2 && (!form.employmentType || !form.monthlyIncomeRange || form.durationMonths === "" || [form.salaryBankCredit, form.itrAvailable, form.gstAvailable, form.udyamAvailable, form.sixMonthBankराज्यment].some((v) => v === null))) return "आय से जुड़े सभी सवालों के जवाब दें।";
    ...
    if (step === 4 && [form.panAvailable, form.aadhaarAvailable, form.addressProofAvailable, form.incomeProofAvailable, form.bankराज्यmentAvailable, form.businessRegistrationAvailable, form.securedAssetAvailable].some((v) => v === null)) return "Documents से जुड़े सभी सवालों के जवाब दें।";
...
type StepProps = { form: Formराज्य; update: <K extends keyof Formराज्य>(key: K, value: Formराज्य[K]) => void };
...
function हाँनहीं<K extends keyof Formराज्य>({ label, field, value, update }: { label: string; field: K; value: boolean | null; update: StepProps["update"] }) {
  return <Field label={label} required><div className="grid grid-cols-2 gap-2"><Choice name={String(field)} value="yes" label="हाँ" checked={value === true} onChange={() => update(field, true as Formराज्य[K])} /><Choice name={String(field)} value="no" label="नहीं" checked={value === false} onChange={() => update(field, false as Formराज्य[K])} /></div></Field>;
}
...
function Safetyनहींte() { return <div className="mt-7 flex items-start gap-3 rounded-2xl border border-brand-100 bg-brand-100/50 p-5 text-sm leading-7 text-brand-700"><LockKeyhole className="mt-1 shrink-0" size={18} />UPI PIN, CVV, net-banking password, Aadhaar OTP या banking details कभी साझा न करें।</div>; }
