...
const steps = ["Profile", "Income", "Credit", "Documents", "Consent"];
...
function BasicStep({ form, update, otpStarted, otpVerified, busy, requestOtp }: StepProps & { otpStarted: boolean; otpVerified: boolean; busy: boolean; requestOtp: () => void }) {
  return <div><StepTitle title="Build your borrower profile" description="Tell us what you need. Your email OTP protects this assessment." /><div className="mt-7 grid gap-5 sm:grid-cols-2"><Field label="Full name" required><Input autoComplete="name" value={form.fullName} onChange={(e) => update("fullName", e.target.value)} placeholder="Your full name" /></Field><Field label="WhatsApp mobile number" required hint="10-digit Indian mobile number"><Input className="number-field" inputMode="numeric" autoComplete="tel" maxLength={10} value={form.mobile} disabled={otpVerified} onChange={(e) => update("mobile", e.target.value.replace(/\D/g, "").slice(0, 10))} placeholder="98XXXXXXXX" /></Field><Field label="Email address" required hint="We will send the OTP to this email"><div className="flex gap-2"><Input type="email" autoComplete="email" value={form.email} disabled={otpVerified} onChange={(e) => update("email", e.target.value.trim())} placeholder="name@example.com" />{!otpVerified ? <Button type="button" variant="secondary" className="shrink-0" disabled={busy} onClick={requestOtp}>{busy ? <Loader2 className="animate-spin" size={17} /> : null}{otpStarted ? "Resend OTP" : "Verify email"}</Button> : <span className="grid min-w-12 place-items-center rounded-xl bg-brand-100 text-brand-700"><Check size={19} /></span>}</div>{otpStarted && !otpVerified ? <p className="mt-2 text-xs text-slate-500">Enter the email OTP in the secure MSG91 window to complete verification.</p> : null}</Field><div className="hidden sm:block" /><Field label="State" required><Input autoComplete="address-level1" value={form.state} onChange={(e) => update("state", e.target.value)} placeholder="e.g. Uttar Pradesh" /></Field><Field label="City" required><Input autoComplete="address-level2" value={form.city} onChange={(e) => update("city", e.target.value)} placeholder="e.g. Greater Noida" /></Field><Field label="Residential address" required><Input autoComplete="street-address" value={form.residentialAddress} onChange={(e) => update("residentialAddress", e.target.value)} placeholder="House / building, area and landmark" /></Field><Field label="PIN code" required><Input inputMode="numeric" maxLength={6} value={form.pinCode} onChange={(e) => update("pinCode", e.target.value.replace(/\\D/g, "").slice(0, 6))} placeholder="201009" /></Field><Field label="Required loan amount" required hint="₹50,000 to ₹15,00,000"><Input className="number-field" type="number" inputMode="numeric" min={50000} max={1500000} step={10000} value={form.loanAmount} onChange={(e) => update("loanAmount", e.target.value)} placeholder="500000" /></Field><Field label="Loan purpose" required><Input value={form.loanPurpose} onChange={(e) => update("loanPurpose", e.target.value)} placeholder="e.g. business expansion" /></Field><Field label="Loan category" required><Select value={form.loanType} onChange={(e) => update("loanType", e.target.value)}><option value="">Choose a category</option><option value="PERSONAL">Personal Loan</option><option value="BUSINESS">Business Loan</option><option value="MSME">MSME Loan</option><option value="MUDRA_GUIDANCE">Mudra Loan Guidance</option><option value="GOLD">Gold Loan</option><option value="PROPERTY">Loan Against Property</option><option value="CREDIT_HEALTH">Credit Health Support</option></Select></Field></div><SafetyNote /></div>;
}

function IncomeStep({ form, update }: StepProps) {
  return <div><StepTitle title="Income and employment profile" description="No payslip or bank-statement upload is required at this stage." /><div className="mt-7 grid gap-5 sm:grid-cols-2"><Field label="Employment type" required><Select value={form.employmentType} onChange={(e) => update("employmentType", e.target.value)}><option value="">Choose one</option><option value="SALARIED">Salaried</option><option value="SELF_EMPLOYED">Self-employed</option><option value="BUSINESS_OWNER">Business owner</option><option value="FREELANCER">Freelancer</option><option value="OTHER">Other</option></Select></Field><Field label="Employer / business name" required><Input value={form.employerOrBusinessName} onChange={(e) => update("employerOrBusinessName", e.target.value)} placeholder="Company or business name" /></Field><Field label="Office / business address" required><Input value={form.officeAddress} onChange={(e) => update("officeAddress", e.target.value)} placeholder="Office address or work location" /></Field><Field label="Monthly net income" required><Select value={form.monthlyIncomeRange} onChange={(e) => update("monthlyIncomeRange", e.target.value)}><option value="">Choose an income range</option><option value="BELOW_15000">Below ₹15,000</option><option value="15000_24999">₹15,000–₹24,999</option><option value="25000_39999">₹25,000–₹39,999</option><option value="40000_59999">₹40,000–₹59,999</option><option value="60000_99999">₹60,000–₹99,999</option><option value="100000_149999">₹1,00,000–₹1,49,999</option><option value="150000_PLUS">₹1,50,000+</option></Select></Field><Field label="Annual income" required><Input className="number-field" type="number" min={0} value={form.annualIncome} onChange={(e) => update("annualIncome", e.target.value)} placeholder="e.g. 800000" /></Field><Field label="Employment or business vintage (months)" required><Input className="number-field" type="number" min={0} max={600} value={form.durationMonths} onChange={(e) => update("durationMonths", e.target.value)} placeholder="e.g. 24" /></Field></div><div className="mt-7 grid gap-5 sm:grid-cols-2"><YesNo label="Is your salary credited to a bank account?" field="salaryBankCredit" value={form.salaryBankCredit} update={update} /><YesNo label="Do you have an ITR?" field="itrAvailable" value={form.itrAvailable} update={update} /><YesNo label="Do you have GST registration or returns?" field="gstAvailable" value={form.gstAvailable} update={update} /><YesNo label="Do you have Udyam registration?" field="udyamAvailable" value={form.udyamAvailable} update={update} /><YesNo label="Do you have a 6-month bank statement?" field="sixMonthBankStatement" value={form.sixMonthBankStatement} update={update} /></div></div>;
}

function ObligationStep({ form, update }: StepProps) {
  return <div><StepTitle title="Credit profile and current obligations" description="Provide a fair estimate. This is not a credit-bureau report or lender verification." /><div className="mt-7 grid gap-5 sm:grid-cols-3"><Field label="Total existing monthly EMI"><Input className="number-field" type="number" min={0} value={form.existingEmi} onChange={(e) => update("existingEmi", e.target.value)} /></Field><Field label="Number of active loans"><Input className="number-field" type="number" min={0} max={100} value={form.activeLoans} onChange={(e) => update("activeLoans", e.target.value)} /></Field><Field label="Credit-card outstanding"><Input className="number-field" type="number" min={0} value={form.cardOutstanding} onChange={(e) => update("cardOutstanding", e.target.value)} /></Field></div><div className="mt-7 grid gap-5 sm:grid-cols-2"><YesNo label="Do you currently have any overdue payment?" field="currentOverdue" value={form.currentOverdue} update={update} /><YesNo label="Has any account been settled or written off?" field="settledOrWrittenOff" value={form.settledOrWrittenOff} update={update} /></div><div className="mt-7"><Field label="Your estimated CIBIL score range" required hint="Choose “I don’t know” if you have not checked an official bureau report."><Select value={form.creditRange} onChange={(e) => update("creditRange", e.target.value)}><option value="">Choose an income range</option><option value="BELOW_550">Below 550</option><option value="550_599">550–599</option><option value="600_649">600–649</option><option value="650_699">650–699</option><option value="700_749">700–749</option><option value="750_PLUS">750+</option><option value="UNKNOWN">अभी पता No</option></Select></Field></div></div>;
}
