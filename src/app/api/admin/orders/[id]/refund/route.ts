...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await prisma.$transaction([
      ...
      prisma.auditLog.create({ data: { adminId: admin.id, action: "PAYMENT_REFUND_COMPLETED", entityType: "Order", entityId: order.id, metadata: { refundId: refund.id, amount: parsed.data.amount, reason: parsed.data.reason, paymentProvider: payment.provider }, ipHash: requestIpHash(request), userAgent: request.headers.get("user-agent")?.slice(0, 500) } }),
    ]);
    ...
    const status = message.includes("prepared") || message.includes("placeholder") ? 501 : 502;
