...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const emailToken = await verifyAccessToken(parsed.data.verificationToken, "otp_verified");
      const tokenEmail = typeof (emailToken as { email?: unknown }).email === "string" ? String((emailToken as { email?: string }).email).toLowerCase() : "";
      if (emailToken.sub === mobile && typeof emailToken.leadId === "string" && tokenEmail === email) {
        leadId = emailToken.leadId;
      }
    } catch (error) {
      if (!isAccessTokenError(error)) throw error;
    }
    if (!leadId) return NextResponse.json({ error: "Complete email OTP verification again." }, { status: 403 });
