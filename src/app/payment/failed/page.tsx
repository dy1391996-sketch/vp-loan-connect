...
import { quickApplyResumePath } from "@/lib/apply/funnel-route";
...
export default async function PaymentFailedPage({ searchParams }: { searchParams: Promise<{ assessment?: string; product?: string; token?: string; reason?: string }> }) {
  ...
  const reason = (query.reason || "unspecified").replace(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi, "[email]").slice(0, 80);
  const retry = query.assessment && query.product && query.token
    ...
    : "/apply/quick";
  const resume = query.assessment && query.token ? quickApplyResumePath(query.assessment, query.token) : "/apply/quick";
    ...
    <section className="surface-grid grid min-h-[72vh] place-items-center bg-surface py-16">
      <div className="page-shell">
        <FunnelBeacon
          ...
          dedupeKey={`payfail_${query.assessment || "none"}_${reason}`}
          properties={{ reason }}
        />
