 ...
 * Never use DATABASE_URL as the shadow DB — that leaves tables without a
 * `_prisma_migrations` history and breaks `prisma migrate deploy` (P3005).
 */
const require = createRequire(import.meta.url);
const prismaCli = require.resolve("prisma/build/index.js");
const projectRoot = process.cwd();
const databaseUrl =
  process.env.DATABASE_URL ?? "postgresql://schema:check@127.0.0.1:5432/vplc?schema=public";
